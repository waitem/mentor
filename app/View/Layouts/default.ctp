 ...
 *
 * This version: Copyright (c) 2012-2016 Mark Waite
 *
...
<title><?php if ($this->Session->check('Tenant.name')) {
	echo __('Business Mentoring ') . $this->Session->read('Tenant.name') ; 
} else {
	echo __('Business Mentoring Application'); 
}
...
<body>
	...
	<div id="container">
		<div id="header" class="left">
			<h1><?php 
				if ($this->Session->check('Tenant.name')) {
					echo $this->Html->link( __('Business Mentoring ') . $this->Session->read('Tenant.name'),
		...
		<div id="content">
			...
			<?php echo $this->Session->flash('auth', array('element' => 'default')); ?>
		...
		<div id="footer">
			...
			echo ' | ';
			echo 'Copyright (c) 2012-' . date("Y") . ' Mark Waite';
			echo ' | ';
